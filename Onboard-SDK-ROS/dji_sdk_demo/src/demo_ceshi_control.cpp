...
void imu_callback(const sensor_msgs::Imu::ConstPtr &msg) //myself
{
  ...
  //current_imu= msg->angular_velocity;//提供当前速度
  //current_imu.y = msg->angular_velocity.y;
  //current_imu.z = msg->angular_velocity.z;
  ...
  f_c_v << fixed;
  f_c_v << currentTime10<< setprecision(9) <<" "<< rx  << " " << ry << " " << rz <<endl;
  f_c_v.close();
...
{
  ros::init(argc, argv, "demo_ceshi_control_node");
  ros::NodeHandle nh;
/*
      ...
      //  if(vins_position.z <= 0.4)
       //if(zz_1 <= 1.5) {
        //  Eigen::Vector3d v_I(current_vel.x, current_vel.y, current_vel.z);
         // current_vel.z = current_vel.z + 0.1;
         // cout<<" current_vel.z= " << current_vel.z << endl;
         // zz_1 = zz_1 + current_vel.z * dt;
         // if(zz_1<=1.5)
         // {
         //   zz_1 = zz_1;
         // }
...
void local_position_ctrl1() 
{
 ...
 // cout << "p_d = " << p_I_x << "  "<< p_I_y << "  "<< p_I_z << endl;
        //   if(zz_1 <= 1.5) {
        //   Eigen::Vector3d v_I(current_vel.x, current_vel.y, current_vel.z);
        //   current_vel.z = current_vel.z + 0.5;
        //   cout<<" current_vel.z111111= " << current_vel.z << endl;
        //   zz_1 = zz_1 + current_vel.z * dt;
        //   if(zz_1<=1.5)
        //   {
        //     zz_1 = zz_1;
...
//  Eigen::Vector3d p_I(p_I_x, p_I_y, p_I_z);//使用dji模拟器时的当前位置提供
  ...
  //Eigen::Vector3d p_I(p_I_x_, p_I_y_, p_I_z_);//使用vins-mono时的当前位置提供
  Eigen::Vector3d v_II(current_vel.x, current_vel.y, current_vel.z);

  current_vel.x = 0;
  current_vel.y = 0;
  current_vel.z = current_vel.z + 0.1;

  if(current_vel.z >= 0.2){
    current_vel.z = 0.2;
  }
  cout << "current_vel.z22222= " << current_vel.z << endl;
  zzz_1 = zzz_1 + current_vel.z * dt;
  if(zzz_1 >= 0.2){
    zzz_1 = 0.2;
  }
  cout << " zzz_1= " << zzz_1 << endl;
  Eigen::Vector3d p_I(0, 0, zzz_1);
  //Eigen::Vector3d p_I(vins_position.x, vins_position.y, vins_position.z);
  //Eigen::Vector3d p_I(vins_camera_position.x, vins_camera_position.y, vins_camera_position.z);
  cout << "p_I = "  << endl<< p_I <<endl;
  ...
  //Eigen::Vector3d v_I(0, 0, 0);//in mocap
  //Eigen::Vector3d v_I(current_vel.x, current_vel.y, current_vel.z);
  Eigen::Vector3d v_I(0, 0, current_vel.z);
...
void imu_callback(const sensor_msgs::Imu::ConstPtr &msg) //myself
{
  ...
  f_c_v.open(cur_imu.c_str(),ios::app);
  f_c_v << fixed;
  f_c_v << currentTime9 << setprecision(9) <<" "<< imu.x  << " " << imu.y << " " << imu.z <<endl;
  f_c_v.close();
